...
func _try_handle_escape(event: InputEvent) -> bool:
	...
	# 第一优先级：
	# 当前获得焦点的搜索栏有内容时，Esc 只清空该搜索栏。
	if disease_search_bar != null and disease_search_bar.has_focus() and disease_search_bar.text != "":
		_on_search_bar_gui_input(event, disease_search_bar)
		return true

	if formula_search_bar != null and formula_search_bar.has_focus() and formula_search_bar.text != "":
		_on_search_bar_gui_input(event, formula_search_bar)
		return true

	if herb_search_bar != null and herb_search_bar.has_focus() and herb_search_bar.text != "":
		_on_search_bar_gui_input(event, herb_search_bar)
		return true

	# 第二优先级：
	# 搜索栏为空，或焦点不在搜索栏时，Esc 关闭整个行医记考窗口。
